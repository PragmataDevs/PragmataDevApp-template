...
### 1.3.1 Módulo Agente Operativo — F0 construido (2026-08-05), gateway/UI pendientes

> ⚠️ **Actualizado 2026-08-05 (F0).** El discovery de Arquímedes de 2026-07-30 (párrafo original de esta sección) fue el diseño inicial; el discovery más profundo de 2026-08-05 (`data/internal/agente-operativo/plan-rollout-2026-08-05.md`, auditoría de las 6 apps de cliente reales) ajustó nombres y contrato al construir F0. **Lo que sigue YA EXISTE en código** — reemplaza los nombres del diseño original (`sideEffect`→`riskLevel`, `sys_agent_audit`→`agent_action_log`, sin `privileged` como campo aparte). El `agent-gateway` (Edge Function), `AgentBubble` (UI) y el prefill (`useAgentDraft`) siguen sin construir (F1/F3).
...
**Piezas construidas (`packages/core/src/agent/`, TypeScript puro sin React — pensado para que lo consuman tanto los hooks de `src/features/<dominio>/hooks/` como una futura Edge Function Deno):**
- `AgentActionDef<TParams,TResult>` (`types.ts`) — `resourceCode`/`requiredAction` (los MISMOS de `src/config/security/resources.ts`), `kind: 'query'|'navigate'|'prefill'|'mutate'|'rpc'`, `riskLevel: 'safe'|'write'|'financial'|'destructive'`, `paramsSchema?` (compatible estructuralmente con un Zod schema, sin depender de `zod` en el paquete), `execute(params, ctx)` — lo implementa cada app con su propio cliente Supabase (RLS = sesión real del usuario, nunca `service_role`).
- `defineAction()` / `buildManifest(defs, hasPermission)` (`manifest.ts`) — generaliza `buildAssistantActions()` del CRM Objetiva: filtra el catálogo declarativo por lo que el usuario puede hacer de verdad.
- `runAction()` / `requiresConfirmation()` (`policy.ts`) — el choke point de ejecución: re-checa `hasPermission` (defensa en profundidad, nunca confía en que el manifest ya filtró) → aplica la política dura de confirmación (`financial`/`destructive` SIEMPRE confirman, sin excepción de rol; `write` confirma por default) → recién ahí llama `execute()`.
- `evaluateRowConditions()` (`conditions.ts`) — espejo en TS de `public.check_row_conditions()` (SQL) para checks de UI antes de mostrar una acción sobre una fila concreta. No reemplaza RLS.
- `agent_action_log` (tabla, AuditBase completo) — bitácora de cada ejecución/rechazo, con `confirmed_by`/`confirmed_at`. Ver `supabase/migrations/20260805193100_agent_action_log.sql`.

**Pendiente (fuera de F0, no construido):** Edge Function `agent-gateway` (function-calling con Gemini como motor default), `AgentBubble` (UI), convención de prefill (`useAgentDraft`).

**Invariante de seguridad (regla dura, ver también `.claude/skills/rbac-god-user/SKILL.md`):** ningún `execute()` de action recibe cliente `service_role`. Se construye SOLO con la sesión real del usuario (RLS). El LLM (cuando exista el gateway) solo decide intención+parámetros; la ejecución siempre corre con permisos reales.

**Decisiones de producto ya tomadas por Wicho (2026-07-30, no volver a preguntar):**
1. Confirmación humana SIEMPRE en acciones `financial`/`destructive`, incluso si quien invoca es `is_god()`. No hay bypass por nivel de privilegio — es código (`requiresConfirmation()`), no un prompt de LLM.
2. El scoping por `conditions` ("solo mis registros / mi sucursal") se implementa desde F0, no se pospone — hecho vía `public.check_row_conditions()` (helper nuevo y separado; `check_permission()` se dejó intacto a propósito, ver comentario de diseño en la migración). Aditivo: `conditions = '{}'` ⇒ comportamiento idéntico al de antes.
3. Motor LLM abstraído de proveedor (F1, `agent-gateway`). Default de producción para agentes embebidos en apps de cliente: **Gemini**, no Anthropic. Claude queda como opción swappable "premium". La suscripción Claude Code/Max de Wicho es solo para su propio trabajo con Praxia — el agente de un cliente siempre se paga por token.

**Roadmap:** F0 (este núcleo + `check_row_conditions` + `agent_action_log`) **HECHO** → F1 `agent-gateway` (manifest+invoke, Gemini function-calling) + `AgentBubble` genérica → F2 contexto (schema pack + manual/RAG) → F3 prefill (`useAgentDraft`) → F4 execute real con OCC, probado primero en Pragmata OS (dogfood) → F5 rollout por cliente (orden: CRM Objetiva → SeguBros → LawRank → IndPack → Clibsa → Objetiva Ops).
